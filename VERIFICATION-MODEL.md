...
| **Evidence / integrity** | evidence: `content_hash`, `sealed_object`, `cbom_chain`, `wayback`, `sbom`, `ai_sbom` | **Real SHA-256.** Re-hash the bytes and compare; chains link child `input_hash` to parent `content_hash`. | `tampered-byte` — one byte changed → hash mismatch → must fail |
| **Policy / enforcement & timing** | security: `intent_policy`, `capability_gate`, `freshness`, `snaft_rule`, `cortex_consent`, `airlock_posture`, `null_route`, `verdict_event`, `no_fail_open` | **Structural & deterministic.** Re-run the decision rule over the inputs; the allow/deny/quarantine/re-attest verdict is a pure function of the record. | the failing/edge case (`no-fail-open`, `stale-no-reattest`, …) → must fail closed |
